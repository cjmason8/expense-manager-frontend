...
const gotoDate: string | null = route.params.date as string
...
if (gotoDate)
  expenseStore.getTransactionsForWeek(gotoDate)
else
...
const saveAddEdit = () => {
  ...
  if (startDate != null)
    selectedItem.value.startDateString = format(startDate, 'dd-MM-yyyy')

  if (endDate != null)
    selectedItem.value.endDateString = format(endDate, 'dd-MM-yyyy')
  ...
  if (recurringTypeId != null) {
    selectedItem.value.recurringType = recurringTypes.value.find(
  ...
  >
    <tr>
      <td>
        <VCard>
          <VCardTitle>
            Week Beginning: {{ expenseStore.homeInfo?.thisWeek }}
          </VCardTitle>
        </VCard>
      </td>
