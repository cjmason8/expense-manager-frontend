...
if [ $? -eq 0 ]; then
  ...
  rancher-compose --url ${RANCHER_URL} --access-key ${RANCHER_ACCESS_KEY} --secret-key ${RANCHER_SECRET_KEY} -e env.txt -p expenseManager -f ${BASE_DIR}/docker-compose-${ENV_NAME}.yml up --confirm-upgrade -d --batch-size 1
else
  echo "Deploy failed :( rolling back..."
  rancher-compose --url ${RANCHER_URL} --access-key ${RANCHER_ACCESS_KEY} --secret-key ${RANCHER_SECRET_KEY} -e env.txt -p expenseManager -f ${BASE_DIR}/docker-compose-${ENV_NAME}.yml up --rollback -d --batch-size 1
fi
