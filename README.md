...
- **Mixed content** mentioning `http://161.97.133.187:8085` while using **HTTPS** on the domain: often **registrar “URL forwarding” / iframe** to that HTTP URL. Use **DNS A record** + reverse proxy instead — see **`docs/mixed-content-domain.md`**.
- **Forward to `http://161.97.133.187` with no port** = **port 80**; Docker uses **8085** on the host — see **`docs/url-forwarding-and-ports.md`**.
